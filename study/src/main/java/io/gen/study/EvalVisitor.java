...
import io.gen.antlr.MuBaseVisitor;
...
public class EvalVisitor extends MuBaseVisitor<Value> {
    ...
    public Value visitPowE(MuParser.PowExprContext ctx){
    ...
    public Value visitAdditiveExpr(@NotNull MuParser.AdditiveExprContext ctx){
        ...
        switch (ctx.op.getType()){
            case MuParser.PLUS:
                return left.isDouble() && right.isDouble() ?  new Value(left.asDouble() + right.asDouble()) : new Value(left.asString() + right.asString());
            case MuParser.MINUS:
                return new Value(left.asDouble() - right.asDouble());
            default:
                throw new RuntimeException("unknown operator: " + MuParser.tokenNames[ctx.op.getType()]);
    ...
    public Value visitWhile(MuParser.While_statContext ctx){
